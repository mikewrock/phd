         ...
         phd::trajectory_service::Response &res)
{
	...
	BOOST_FOREACH (pcl::PointXYZI& p, cloud->points){
		ROS_INFO("Point");
		if(p.x < minX.x && (p.x+p.y+p.z) != 0){ 
		ROS_INFO("New Point %f-%f-%f",p.x,p.y,p.z);
			minX = p;
		}
	...
	if ( octree.nearestKSearch (start_pt, K, pointIdxNKNSearch, pointNKNSquaredDistance) > 0 ) //make sure we find something
	{
		...
		for (size_t i = 0; i < pointIdxNKNSearch.size (); ++i){
			...
			if(fabs(start_pt.z - cloud->points[ pointIdxNKNSearch[i] ].z) < 0.02){
				//caluclate the distance between points				
...
ROS_INFO("finding pthers");
	...
	for(int i = 0; i < 6; ++i){
		normal_pt[i] = find_normal(via_pt[i]);
		t_pt.x = via_pt[i].x;
		t_pt.y = via_pt[i].y;
		t_pt.z = via_pt[i].z;
		t_pt.nx = normal_pt[i].x;
		t_pt.ny = normal_pt[i].y;
		t_pt.nz = normal_pt[i].z;
